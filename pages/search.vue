...
const { data, error } = await useFetch<ApiResponse<Product[]>>(`${apiBaseUrl}/product/search?keyword=${route.query.keyword}&sort=${sort.value}&order=${order.value}&startIdx=${startIdx.value}`,
    {
        method: "GET",
    })
if (error.value) {
...
const load = async () => {
    ...
    try {
        // isLoading = true
        await $fetch(`${apiBaseUrl}/product/search?keyword=${route.query.keyword}&sort=${sort.value}&order=${order.value}&startIdx=${startIdx.value}`, {
            method: "GET",
            onRequest() {
                loading.value = true
            },
